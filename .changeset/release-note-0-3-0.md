...
**The 0.3.0 release note is written from measurements rather than intentions.**

What the first `sync` writes on a repository materialized by 0.1.x is counted across four
repositories this tool did not build, not estimated: 118 paths `keep`, 24 `update`, 54 `conflict`,
and two offered for writing. All seven classes occurred there, `orphaned` and `removed` included —
