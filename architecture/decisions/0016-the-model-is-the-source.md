...
absent from the model (L3, lands with the projection, when `doctor` first reads the model).
...
consumer at the projection step it gains a real surface, and that step's acceptance carries the
mutation gate that raises it to L3: make `doctor` derive ownership from anything other than `authoredBy` — from what references
