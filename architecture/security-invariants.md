...
| `doctor` executes nothing from the repository it inspects: it derives every verdict from reading file text over a declared read set, so auditing an untrusted clone never runs that clone's code (and no verdict can claim L4) | The `spawnPolicy` `no-restricted-syntax` block in `eslint.config.mjs` forbids `node:child_process`, `node:module`, `import()`, `require`, `require.resolve` and `createRequire` under `src/**`; `tests/dependency-policy.test.ts` lints one sample per form; `tests/doctor-checks.test.ts` asserts no verdict is L4 |
